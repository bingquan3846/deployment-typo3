 ...
 * Include config
 */
$deploymentConfigPathAndFilename = __DIR__ . '/ReviewDeployment/config.php';
...
$setadditionalconfigurationOptions = array(
    'command' => '
		echo ' . escapeshellarg(str_replace('###CI_BUILD_REF_NAME###', getenv('CI_BUILD_REF_NAME'), file_get_contents(__DIR__ . '/ReviewDeployment/AdditionalConfiguration.php'))) . ' > {workspacePath}/typo3conf/AdditionalConfiguration.php',
);
...
/**
 * Set RealURL Local.php
 *
 * LocalRealUrl.php has to exist and has to contain at least "<?php if (!defined ('TYPO3_MODE')) die('Access denied.'); ?>"
 */
$setlocalrealurlOptions = array(
    'command' => '
		echo ' . escapeshellarg(str_replace('###CI_BUILD_REF_NAME###', getenv('CI_BUILD_REF_NAME'), file_get_contents(__DIR__ . '/ReviewDeployment/LocalRealUrl.php'))) . ' > {workspacePath}/typo3conf/ext/bgm_theme_bgm/Configuration/RealUrl/Local.php',
);
$workflow->defineTask('setlocalrealurl', 'TYPO3\\Surf\\Task\\LocalShellTask', $setlocalrealurlOptions);
$workflow->afterStage('package', 'setlocalrealurl');
...
$setinisetOptions = array(
    'command' => '
		echo ' . escapeshellarg(str_replace('###CI_BUILD_REF_NAME###', getenv('CI_BUILD_REF_NAME'), file_get_contents(__DIR__ . '/ReviewDeployment/iniset.php'))) . ' > {releasePath}/typo3conf/iniset.php;
		echo ' . escapeshellarg(str_replace('###CI_BUILD_REF_NAME###', getenv('CI_BUILD_REF_NAME'), file_get_contents(__DIR__ . '/ReviewDeployment/iniset.php'))) . ' > {currentPath}/typo3conf/iniset.php',
    'rollbackCommand' => '
...
$settagOptions = [
    'command' => '
		git tag -f -a -m ' . escapeshellarg('DeploymentName: ' . $deploymentName . '; ReleaseIdentifier: ' . $deployment->getReleaseIdentifier() . ';' . (getenv('CI_BUILD_ID') ? ' CI_BUILD_ID: ' . getenv('CI_BUILD_ID') . ';' : '') . (getenv('CI_BUILD_REF_NAME') ? ' CI_BUILD_REF_NAME: ' . getenv('CI_BUILD_REF_NAME') . ';' : '')) . ' ' . escapeshellarg($deploymentName . '-' . $deployment->getReleaseIdentifier() . (getenv('CI_BUILD_ID') ? '-' . getenv('CI_BUILD_ID') : '') . (getenv('CI_BUILD_REF_NAME') ? '-' . getenv('CI_BUILD_REF_NAME') : '')) . ' && 
		git push origin ' . escapeshellarg($deploymentName . '-' . $deployment->getReleaseIdentifier() . (getenv('CI_BUILD_ID') ? '-' . getenv('CI_BUILD_ID') : '') . (getenv('CI_BUILD_REF_NAME') ? '-' . getenv('CI_BUILD_REF_NAME') : '')),
];
